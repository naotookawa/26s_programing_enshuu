...
class Player(abc.ABC):
    ...
    def initialize(self, field: Field) -> None:
        '''
    ...
    def move(self, ship_type, to) -> dict:
        '''移動の処理を行い，連想配列で結果を返す．'''
    ...
    def overlap(self, position) -> Ship | None:
        '''与えられた座標にいる艦を返す．'''
...
def play_game(host: str, port: int, player: Player) -> None:
    """仕様に従ってサーバとソケット通信を行う．"""
    import socket
    import logging
