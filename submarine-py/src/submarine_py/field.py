...
class Reporter:
    ...
    def report_result(results, c):
        ...
        if "moved" in result2["result"]:
            ...
            if result2["result"]["attacked"]:
                ...
                print(f"player{(c+1)} faild to attack")

        if c == 0:
            print(f"player{(c+1)}: " + result1["observation"]["me"])
            print(f"player{(2-c)}: " + result2["observation"]["me"])
        else:
            print(f"player{(2-c)}: " + result2["observation"]["me"])
            print(f"player{(c+1)}: " + result1["observation"]["me"])
        print()
    ...
    @staticmethod
    def make_view(field: Field, fleets, attacked) -> str:
    ...
    @staticmethod
    def report_field(field: Field, result, c):
        results = [json.loads(result[0]), json.loads(result[1])]
        fleets = [results[c]["observation"]["me"],
                  results[1-c]["observation"]["me"]]
        attacked = None
        result1 = results[1].get('result', False) or {}
        if attacked_dict := result1.get('attacked', None):
            attacked = attacked_dict["position"]
        views = [
